export const PREPARATION_CHECKLIST = [
  {
    category: "Documents & visa",
    items: [
      "Passport valid for your full stay — check the expiry date now, some airlines ask for extra margin beyond your trip.",
      "Confirm visa requirements for your nationality. EU/Spanish passport holders can typically enter Japan visa-free for tourism up to 90 days, but double-check current rules before flying.",
      "Register on Visit Japan Web ahead of time — it speeds up immigration and customs with a QR code.",
      "Save digital and printed copies of your flight itinerary, hotel bookings, and travel insurance policy.",
      "Photograph your passport and insurance documents and store them somewhere accessible offline.",
    ],
  },
  {
    category: "Money",
    items: [
      "Activate your Revolut card and top up a JPY balance in advance to lock in the exchange rate.",
      "Check Revolut's ATM withdrawal limits and fees for Japan, and raise your card limits if needed.",
      "Bring a backup card from a different bank or network in case Revolut has an issue abroad.",
      "Withdraw some cash on arrival — Japan is still very cash-based, especially at temples, small shops, and rural spots.",
      "7-Eleven and Japan Post ATMs are the most reliable for foreign cards.",
    ],
  },
  {
    category: "SIM & connectivity",
    items: [
      "Buy an eSIM for Japan (e.g. Airalo, Ubigi) and install it before you fly so it's ready to activate on arrival.",
      "Alternatively, reserve a pocket WiFi or physical SIM for pickup at Narita.",
      "Download offline maps for Tokyo and Kyoto in case you're without signal.",
    ],
  },
  {
    category: "Apps to install",
    items: [
      "Google Maps — download the offline maps for Tokyo and Kyoto.",
      "Google Translate — download the Japanese offline pack and try the camera translation feature.",
      "Visit Japan Web — for the immigration and customs QR code.",
      "Revolut — for card management and currency exchange.",
      "A transit app (Google Maps or Japan Transit Planner) for train times and platform info.",
      "Suica or PASMO in Apple Wallet / Google Wallet, if your phone supports it, for tap-to-pay on trains.",
    ],
  },
];
